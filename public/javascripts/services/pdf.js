app.factory("Pdf", ['Network', '$http', function ($network, $http) {
        ...
        */
	    for(var i=0; i<tickets.length; i++) {
			...
			if(ticket.titre.indexOf('-') > -1) {
				...
				doc.text(PADDING + 50, 10 + (quart_page * nb_quart_page), splitedTitle[0]);
				doc.setFontStyle('normal');
				doc.text(PADDING - 15, 20 + (quart_page * nb_quart_page), splitedTitle[1]);
			} else {
				doc.setFontStyle('bold');
